...
- **PDF**: `reportlab` (informes). Los **gráficos del PDF se dibujan con `reportlab.graphics`** (Pie + HorizontalBarChart), NO con plotly→imagen. ⚠️ **Decisión (no reintroducir kaleido sin revisar esto)**: `plotly 6` + `kaleido` es incompatible (kaleido 0.2.1 está deprecado para plotly 6 y `fig.to_image()` se **cuelga**; kaleido 1.x exigiría instalar Chrome en Streamlit Cloud). Por eso se descartó kaleido y se dibujan los gráficos en vectorial con reportlab. La construcción de cada gráfico va en `try/except` (si fallara, el PDF se genera con "(gráfico no disponible)"). `pillow` no se fija (lo aporta Streamlit).
...
- `src/informes_pdf.py` — generación de informes PDF con **reportlab**. Dos capas: infraestructura reutilizable (`estilos()`, `cabecera_flowables()` con los logos, `pie_factory()`, `tabla_estilizada()`, `construir_pdf()`) y dos informes concretos: `generar_pdf_resumen(plan_id, idioma)` (Cuadro resumen, A4 retrato: cabecera, métricas, tarta de estados + barras por ámbito dibujadas con **reportlab.graphics**, y las dos tablas con semáforo) y `generar_pdf_coordinacion(plan_id, idioma, df_filtrado, filtros_desc)` (Registros de coordinación, **A4 apaisado**: línea de filtros + tabla con `repeatRows`, recibe el DataFrame YA filtrado). `construir_pdf(..., apaisado=False)` admite ambas orientaciones. Reutilizan las consultas y `obtener_plan`. Los gráficos van en `try/except`: si fallaran, "(gráfico no disponible)". NO usa kaleido (ver Stack: plotly 6 + kaleido incompatible).
...
- ~~Exportación a PDF del Resumen del Plan~~ (hecho: pestaña "Cuadro resumen" → Generar/Descargar PDF, `src/informes_pdf.py`).
- ~~PDF de los registros de Coordinación~~ (hecho: pestaña "Coordinación" → Generar/Descargar PDF apaisado de la tabla filtrada, `generar_pdf_coordinacion`).
