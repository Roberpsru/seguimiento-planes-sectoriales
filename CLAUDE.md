...
- `src/informes_pdf.py` — generación de informes PDF con **reportlab**. Dos capas: infraestructura reutilizable (`estilos()`, `cabecera_flowables()` con los logos, `pie_factory()`, `tabla_estilizada()`, `construir_pdf()`) y el informe concreto `generar_pdf_resumen(plan_id, idioma)` (Cuadro resumen: cabecera, métricas, tarta de estados + barras por ámbito dibujadas con **reportlab.graphics**, y las dos tablas con semáforo). Reutiliza las consultas de resumen y `obtener_plan`. Cada gráfico va en `try/except`: si fallara, el PDF se genera con "(gráfico no disponible)". NO usa kaleido (ver Stack: plotly 6 + kaleido incompatible). El futuro PDF de Coordinación debe reutilizar la infraestructura común.
...
- ~~Exportación a PDF del Resumen del Plan~~ (hecho: pestaña "Cuadro resumen" → Generar/Descargar PDF, `src/informes_pdf.py`). Pendiente: PDF de los registros de Coordinación (reutilizando la infraestructura de `informes_pdf.py`).
